...
Read `.claude/docs/technical-preferences.md` and the pinned Unity/render-pipeline
references when present; extract asset/import/performance constraints.
...
- **`technical-artist`**: Unity/render-pipeline hard constraints — poly count
  budgets, texture memory, material slots, importer settings, and measured
  performance budgets
...
- Is the Unity project/version/toolchain validated by `/setup-engine unity --audit`? → skip that option
...
- `[_] Run /setup-engine unity --audit — validate Unity/render-pipeline constraints` (skip if validated)
...
- Run `/setup-engine unity --audit` if Unity/render-pipeline constraints are not yet validated
