...
Write findings to `docs/tech-debt-register.md` when that register is the
declared output path for this invocation.

Append new findings without overwriting existing entries. Verdict:
**COMPLETE** — scan findings written to the register.
...
Append the requested entry as the declared operation of `add`. Verdict:
**COMPLETE** — entry added to the register.
...
After the reprioritization decision is approved, update
`docs/tech-debt-register.md` without a second file prompt.

Write the approved ordering. Verdict: **COMPLETE** — register re-prioritized
and saved. If the reprioritization decision is declined, return **BLOCKED**.
