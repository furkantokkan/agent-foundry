...
- [ ] Concept prototype evidence exists at
      `production/prototypes/[name]-concept/REPORT.md` with a PROCEED verdict;
      any Unity implementation stays under `Assets/Prototypes/**`
...
- [ ] Unity Test Framework initialized with EditMode and PlayMode test assemblies under `Assets/Tests/` or documented feature-local paths
...
**Required Artifacts:**
- [ ] Vertical-slice evidence exists at
      `production/prototypes/[name]-vertical-slice/REPORT.md` (run
      `/vertical-slice`) — **recommended, not blocking**; if absent, surface as
      CONCERNS
...
**Required Artifacts:**
- [ ] `Assets/Game/` has active C# code organized into feature/assembly boundaries
- [ ] All core mechanics from GDD are implemented (cross-reference `design/gdd/` with `Assets/Game/`)
- [ ] Main gameplay path is playable end-to-end
- [ ] EditMode/PlayMode test files exist under `Assets/Tests/` or feature-local test assemblies for Logic and Integration stories
- [ ] All Logic stories from this sprint have corresponding passing EditMode tests
...
- [ ] Localization strings are externalized (no hardcoded player-facing text in `Assets/Game/`)
...
- For performance checks: `Read` technical-preferences.md and compare against
  captured profiler/device evidence in `production/qa/performance/` or recent
  `/perf-profile` output
- For localization checks: `Grep` for hardcoded strings in `Assets/Game/`

### Cross-Reference Checks
- Compare `design/gdd/` documents against `Assets/Game/` implementations
...
- [ ] Tests — FAILED (3 failures in Unity EditMode results)
...
2. **3 test failures** — Fix failing Unity EditMode tests before advancing.
...
**Always obtain the stage-transition decision**: ask whether the project should
advance to Production. If approved, update `production/stage.txt` as part of
that decision; do not ask a second file-write question.
...
5. **Write report**: persist the gate report under `production/gate-checks/**`
   as the declared low-risk output; stage changes remain decision-gated
