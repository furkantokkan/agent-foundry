...
## Step 0: Gather Live Project State

Use Read/Glob to inspect:
- `production/stage.txt` if present
- latest `production/sprints/*.md` file if present
- first section of `production/session-state/active.md` if present

If any are missing, treat them as "not set" rather than an error.
...
2. **If stage.txt is missing**, infer phase from artifacts (most-advanced match wins):
   - `Assets/Game/` has 10+ C# source files → `production`
