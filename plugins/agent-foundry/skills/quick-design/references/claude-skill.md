...
- If this is a Tuning change, also check `Assets/Game/Data/` or the documented
  feature-local ScriptableObject/data path that holds the relevant values.
...
- [ ] [Parameter] reads [new value] from `Assets/Game/Data/[asset-or-file]`
...
All tunable values must live in the owning ScriptableObject/data asset under
`Assets/Game/Data/` (or its documented feature-local path), not be hardcoded.
...
If [A]: after the quick-design decision is approved, write
`design/quick-specs/[kebab-case-title]-[YYYY-MM-DD].md` as the declared output.
...
"This spec modifies rules in [System Name]. Approve the proposed update to the
[section name] section of `design/gdd/[filename].md`?" After approval, persist
that decision without a separate file-write prompt.
