...
2. **Check for existing concept work**:
   ...
   - Read `.claude/docs/reference-libraries.md` if it exists. If
     `docs/reference/private/game-design/README.md` exists, read only that
     index before ideation. Use targeted Grep later for relevant design theory,
     postmortems, or examples; do not bulk-read full books or OCR JSON.
...
- **Target platform**: Use `AskUserQuestion` — "What platforms are you targeting for this game?"
  ...
  Record the answer — it shapes Unity platform modules, rendering/input/UI,
  performance budgets, build automation, and package validation passed to
  `/setup-engine unity --audit`.

- **Unity/team experience**: Ask which Unity workflows/packages/platforms the
  team already knows and which project constraints are fixed. Do not offer an
  engine selection; this template is Unity-only.
...
Pass: core loop description, platform target, pinned Unity/project status,
team experience, and identified technical risks.
...
5. Use `AskUserQuestion` for final concept approval; that decision also
   authorizes the declared concept-document write:
- After the user approves the concept, write it to
  `design/gdd/game-concept.md`; do not request separate file permission.
...
**Path A — Design-First** (recommended if the concept is well-defined):
   1. "Run `/setup-engine unity --audit` to validate the pinned Unity project, canonical stack, and references"
...
**Path B — Prototype-First** (use if the core mechanic is unproven or the concept needs validation):
   1. "Run `/setup-engine unity --audit` to validate the Unity prototype environment"
...
7. **Output a summary** with the chosen concept's elevator pitch, pillars,
   primary player type, Unity/platform constraints, biggest risk, and file path.
...
1. `/setup-engine unity --audit` — validate the pinned Unity project and version-aware references
