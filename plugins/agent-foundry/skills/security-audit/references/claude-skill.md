...
- A manifest of `Assets/Game/`, relevant data under `Assets/`, `Packages/`,
  `ProjectSettings/`, and external/backend configuration paths
...
- Are any API keys, credentials, or secrets hardcoded in `Assets/`, `Packages/`,
  `ProjectSettings/`, build scripts, or configuration files?
...
`production/security/security-audit-[date].md` as this read-only audit's
declared output; do not apply fixes unless separately delegated.
