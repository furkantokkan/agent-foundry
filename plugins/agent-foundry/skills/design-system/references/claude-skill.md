...
- **Reference library**: Read `.claude/docs/reference-libraries.md` if present.
  If `docs/reference/private/game-design/README.md` exists, read the index only,
  then use targeted Grep for the system domain (e.g. economy, pacing, fail-state,
  AI, level, loop, pressure). Summarize applicable lessons; do not quote or copy
  long passages into the GDD.
...
Use `AskUserQuestion`:
- "Approve this design scope and output contract? Accepted sections will be
  persisted to `design/gdd/[system-name].md`."
  - Options: "Approve and start", "Show me more context first", "Design a dependency first"

---

## 3. Create File Skeleton

Once the design scope and output contract are approved, **immediately** create
the GDD file with empty section headers. That approval authorizes the low-risk
skeleton and each subsequently accepted section at the declared path; do not
ask a second file-write question. This ensures incremental writes have a target.
...
arise during the Overview (e.g., "Should this use a plain C# service or the
approved typed event boundary?"), note them as "→ becomes an ADR" and move on. Implementation patterns belong
...
Show the exact registry change and obtain approval for the shared-registry plan.
After approval, update `design/registry/entities.yaml` with the [N] new entries
and `referenced_by` changes without a second file prompt.
...
Update `design/gdd/systems-index.md` as part of the approved design-system
write plan.
...
2. **AskUserQuestion** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 3: persist the skeleton under the approved output contract
   - Phase 4 (each section): Design questions, approach options, draft approval
   - Phase 5: "Run design review? Update systems index? What's next?"
3. **One scope approval, then autonomous persistence** — approve material design
   decisions, then write the skeleton and accepted sections to the declared path
   without per-section file prompts
