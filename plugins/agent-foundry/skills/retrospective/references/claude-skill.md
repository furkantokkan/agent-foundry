...
Write the approved retrospective to
`production/retrospectives/retro-sprint-[N]-[date].md` (or the milestone path)
as this invocation's declared output.

Write the file, creating `production/retrospectives/` if needed. Verdict:
**COMPLETE** — retrospective saved. Return **BLOCKED** only when required sprint
evidence or the declared output scope is unavailable.
