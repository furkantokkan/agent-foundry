...
**Output:** Conflict report. Registry corrections are a separate approved
propagation task, not an automatic side effect of this check.
...
This invocation authorizes only its declared conflict report and session note.
Do not modify GDDs or the registry while performing the check.

If stale registry entries are found, include an exact proposed diff for
`design/registry/entities.yaml` and route it to `/propagate-design-change` or a
separately approved registry-maintenance task.
...
append each conflict to `docs/consistency-failures.md` as this invocation's
declared low-risk report output:
...
Then append the new conflict entries. If approval is denied, include the conflict
entries in the response so they are not lost.

---

## Phase 7: Session State and Closing

Write the consistency-check session note to
`production/session-state/active.md` as the declared companion output.

If approved, append the note (create the file if it does not exist):
