...
- After the propagation plan is approved, update the status in the owned ADR.
...
Update the traceability index as part of the approved propagation plan.

---

## 9. Output Change Impact Document

Write the change impact report to
`docs/architecture/change-impact-[date]-[system-slug].md` as the declared
artifact after the propagation plan is approved.
...
If the propagation plan was approved: Verdict: **COMPLETE** — change impact
report saved. If the plan was declined: Verdict: **BLOCKED** — propagation
decision declined.
...
4. **Approve the propagation plan once** — show exact affected files/diffs,
   receive one medium-risk plan decision, then apply only that scope
