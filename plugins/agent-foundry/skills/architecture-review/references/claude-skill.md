...
| **Cross-system communication** | "Damage system notifies UI and audio simultaneously" → typed event/message boundary ADR |
...
Glob `Assets/Tests/EditMode/**/*.cs`, `Assets/Tests/PlayMode/**/*.cs`, and
feature-local Unity test assemblies.
...
|-------|-----|-------------|-----|-------|-----------|-------------|
| TR-combat-001 | combat.md | Hitbox < 1 frame | ADR-0003 | story-001-hitbox.md | Assets/Tests/EditMode/Combat/HitboxTests.cs | COVERED |
...
- Read the pinned Unity version and Unity specialist routing from the canonical repository docs
- If the Unity project/version cannot be validated, return `BLOCKED`
- Spawn `subagent_type: unity-specialist` with all Unity-specific ADRs,
  pinned references, and Phase 5 findings. Ask them to:
  1. Confirm or challenge each audit finding — specialists may know of engine nuances not captured in the reference docs
  2. Identify Unity anti-patterns the audit may have missed (e.g., domain logic in MonoBehaviours, runtime-to-Editor asmdef references, unsafe serialized ownership)
...
2. **Known engine or package limitations in ADRs**: If an ADR records a verified
   Unity/package limitation (for example, the accepted Addressables topology
   excludes remote catalogs), check GDDs that design mechanics around the
   affected feature.
...
|-----|-----------|--------------------------------------|--------|
| live-events.md | "Download event content after launch" | Remote catalogs are excluded by ADR-0003 | Revise GDD or supersede ADR |
```
...
- Report the [N] revision flags. Update the systems index only after the user
  approves that shared-index plan; do not ask a second file-write question.
...
## Phase 8: Write Report and Gate Shared Traceability Updates

Write `docs/architecture/architecture-review-[date].md` as the declared
low-risk review output. Then show exact diffs for the traceability index and TR
registry. Use `AskUserQuestion` once to approve or defer that shared-index plan;
do not ask separately for each file.

### RTM Output (rtm mode only)

For `rtm` mode, show the full matrix and then write the approved Requirements
Traceability Matrix to `docs/architecture/requirements-traceability.md` as the
declared output. Ask only if a requirement classification is unresolved.
...
Show the exact `docs/architecture/tr-registry.yaml` diff for new requirement IDs.
Apply it only after the user approves that shared-registry plan; no second file
prompt is needed after plan approval.
...
2. **Pre-gate checklist**: Check whether these exist via Glob and mark each ✅ or ❌:
   - Unity EditMode/PlayMode test assemblies under `Assets/Tests/` or documented feature-local paths — if ❌: run `/test-setup`
...
4. **Show material decisions before approval** — display proposed changes to
   shared ADR/index/registry state before asking for plan approval.
5. **Use `AskUserQuestion` for material decisions and shared-registry plans**;
   declared low-risk report files do not need duplicate approval. Group a
   multi-file shared-state plan into one decision.
