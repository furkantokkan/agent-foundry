...
Use `AskUserQuestion`:
- Once the manifest decisions are approved, persist the Control Manifest in its
  declared path without a separate write prompt.
- Options:
...
3. **Approve manifest decisions once** — after the input/control decisions are
   approved, create or update only the declared manifest path. Verdict:
   **COMPLETE**. If the decisions are declined, Verdict: **BLOCKED**.
