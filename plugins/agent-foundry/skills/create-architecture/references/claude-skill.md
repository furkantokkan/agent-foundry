...
5. `.claude/docs/reference-libraries.md` if present. If
   `docs/reference/private/coding/README.md` exists, read the index and use
   targeted Grep only for relevant architecture patterns, dependency injection,
   async, Unity editor tooling, or library choices. Treat these as supporting
...
- **Unity APIs used**: which specific Unity classes/components/events this module
  calls directly (with version and risk level noted)

Format as a table per layer, then as an ASCII dependency diagram.

**Unity awareness check**: For every Unity API listed, verify against the pinned
Unity/package reference. If an API is post-cutoff, flag it:

```
⚠️  [ClassName.Method()] — Unity [pinned version] (post-cutoff, HIGH risk)
    Verified against: docs/engine-reference/unity/[reference].md
...
2. **Typed event/message path**: How systems communicate without tight coupling
...
- State whether this is a synchronous interface call, approved typed event, or owned state
...
- What are the entry points (methods/events/properties)?
...
**Unity awareness check**: Keep domain contracts free of Unity types where
possible. When an interface must use `GameObject`, `Component`, `ScriptableObject`,
or another Unity/package type, flag and verify the pinned API/version.
...
- [e.g. "Typed event boundary vs direct interface call"]
...
- After all sections are approved, write the master architecture document; the
  section approval already authorizes this declared output.
...
- After sign-off, update the Document Status section as part of the approved
  document operation.
...
> - [ ] Run `/test-setup` — scaffolds Unity EditMode/PlayMode test assemblies, CI guidance, and an example test
...
4. **Draft before decision approval** — show material architecture choices before
   asking the user to select them.
5. **Use `AskUserQuestion` for material architecture decisions**. After those
   decisions are approved, persist declared documents without duplicate
   file-write prompts. Group any medium-risk multi-file plan into one decision.
...
- Run `/test-setup` to scaffold Unity EditMode/PlayMode test assemblies, CI guidance, and an example test (required for gate-check)
