...
- **unity-ui-specialist** — Validates UI Toolkit/UGUI choices and Unity UI implementation patterns
...
- `subagent_type: unity-ui-specialist` — Unity UI pattern validation
- `subagent_type: accessibility-specialist` — Accessibility compliance audit

Always provide full context plus base/worktree, allowed/forbidden paths,
serialized ownership, and output contract. Parallel review may be read-only;
parallel writing requires independent stories, separate worktrees, and disjoint
paths. One writer owns each UXML/USS/prefab/scene/asset mutation.
...
Before implementation begins, spawn **unity-ui-specialist** to review the UX
and visual specs for Unity implementation guidance:
- Should this screen use UI Toolkit or the project's justified existing UGUI boundary?
- Which Unity/package/version constraints affect layout, input, localization, or accessibility?
- What VisualElement/component structure and serialized ownership are required?
...
Low-risk implementation/tests inside approved story paths are autonomous.
Material UI contracts need plan approval; scenes, prefabs, ScriptableObjects,
project settings, and package changes need exact high-risk approval and one writer.
...
- Run `/game-code-review` on the UI implementation before closing stories.
