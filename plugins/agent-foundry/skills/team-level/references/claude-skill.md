...
- Ask level-designer to compile the approved decisions into the level design
  document. The phase gate authorizes this low-risk document write; show the
  resulting diff instead of requesting permission again per file.
...
Each agent receives allowed/forbidden paths and an output contract. Read-only
design passes may run in parallel; only one compiler writes the approved level
document. Unity scenes, prefabs, NavMesh data, lighting data, and other serialized
assets always have one writer and exact high-risk approval.
