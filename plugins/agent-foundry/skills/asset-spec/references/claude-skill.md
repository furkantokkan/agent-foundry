...
After the user approves the inventory decision, write
`design/assets/entity-inventory.md` without a duplicate file prompt.
...
`design/assets/specs/[target-name]-assets.md` within this skill's low-risk scope.
...
Update `design/assets/asset-manifest.md` as the declared companion output after
the spec is approved. Stop first if this would overwrite an unrelated owner.
