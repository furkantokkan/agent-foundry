...
gate (Phase 4) may revise the story list. Both files are written together after
the final plan decision; no separate file approval follows.
...
If UNREALISTIC: revise the story selection (defer stories to Should Have or
Nice to Have) and re-present it for final plan approval.
...
If [A]: persist the approved plan outputs.
If [B]: revise the story list, re-present the updated plan, then persist after approval.
If [C]: adjust sprint dates and capacity, re-present the updated plan, then persist after approval.

After the producer's plan verdict is approved, write
`production/sprints/sprint-[N].md` and `production/sprint-status.yaml` as the
declared outputs without a second file prompt. Verdict: **COMPLETE** — plan and
status created. If the plan decision is declined, Verdict: **BLOCKED**.
