...
- `.claude/docs/technical-preferences.md` — Unity project/version/toolchain validated?
- `docs/engine-reference/unity/` — pinned Unity/package references present?
...
- 10+ C# source files in `Assets/Game/` → Production
...
| Unity reference | `docs/engine-reference/unity/VERSION.md` | HIGH — ADR Unity checks blind |
...
- Unity version, C#, render pipeline, physics → HIGH if unvalidated (ADR skills fail)
...
Unity project/version not validated when ADRs exist.
...
Unity: [validated exact version / NOT VALIDATED]
...
> **Generated**: [date]
> **Project phase**: [phase]
> **Engine**: Unity [exact version, or "Not validated"]
...
3. **Approve the adoption plan once** — after scope and migration decisions are
   approved, persist the declared plan without a second file prompt
