...
Present the rollback plan, then write it to
`production/releases/rollback-plan-[version].md` as the declared documentation
output. Do not execute rollback/release actions without exact approval.
...
- **Broader regression** — run targeted Unity EditMode and PlayMode tests for affected systems
...
Write the patch record to `production/releases/day-one-patch-[version].md` as
the declared artifact; deployment remains separately gated.
