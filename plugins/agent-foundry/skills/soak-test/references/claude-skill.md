...
Read:
- `.claude/docs/technical-preferences.md` — pinned Unity/tooling and performance
  budgets (memory ceiling, target FPS)
...
- Use the pinned Unity Profiler and Memory Profiler package/version documented
  by the project; record the exact editor/device/build target.
- Record Total Reserved/Used Memory, GC allocation rate, managed heap, native
  object count, and feature-specific counters at every checkpoint.
- Compare stabilized checkpoints after load/warm-up. Monotonic growth across
  three or more comparable checkpoints is a leak signal, not proof by itself.
- Retain profiler/memory snapshots and device logs. A narrative observation is
...
- [ ] Performance monitoring tool open and recording:
  - **Unity**: Profiler/Memory Profiler or approved device capture is attached
    to the exact build/project under test
...
| [Unity feature-specific metric] | [record] |
...
Present the protocol summary, then write it to
`production/qa/soak-test-[date]-[duration].md` as the declared low-risk
artifact. Starting a long-running test or external monitor remains separately
authorized.
...
4. Preserve every Issues Found row with its checkpoint, build/revision, logs,
   profiler capture, expected result, and actual result
5. Route task-owned observations with `/task-bug <TASK-ID> "<raw evidence>"`;
   one command may carry several same-task observations and task-cycle will split
   them into linked atomic D-records under one lifecycle writer
6. For an unowned issue, use `/bug-report` only for backlog capture or
   `/create-task` when a bounded repair is explicitly wanted
7. If ownership is ambiguous, stop with `TASK MATCH AMBIGUOUS`; do not allocate
   a D-record, BUG file, or repair task
8. Run `/bug-triage sprint` only for standalone backlog BUG files

If the verdict is FAIL, run `/smoke-check` again after fixing the issues."

The `SOAK-xxx` row is an observation ID inside this aggregate protocol, not a
second canonical bug store. This skill does not invoke repair or mutate task
lifecycle state after the human session.
...
- **Declared output** — create the protocol file without a duplicate prompt;
  starting the soak run remains separately authorized
