...
Search `Assets/Game/` and UXML under `Assets/` for hardcoded user-facing strings:
...
- Compare against the project's declared Unity Localization tables under `Assets/`
...
Present the localization contract and diff. After approval, write the new entries
to the project's declared path under `Assets/Game/Localization/` without asking
again for each file.

Write only the approved diff, not a full replacement. Verdict: **COMPLETE** — strings extracted and written.

---

## Phase 2C: Validate Mode

Read all declared Unity Localization tables under `Assets/`. For each locale, check:
...
- **Orphaned keys** — translation exists but nothing in `Assets/Game/` or UXML references the key
...
- `Assets/Game/Localization/` — discover the source-locale Unity Localization
  String Tables (or the project's documented localization data path)
...
Write the translator brief to
`production/localization/translator-brief-[locale]-[date].md` as the declared
low-risk output.
...
Spawn `localization-lead` via Task. Ask them to audit the following for cultural
sensitivity across the target locales. Read project-owned content from
`Assets/Game/Localization/`, `Assets/Game/Art/`, and the documented narrative
paths; do not sweep vendor assets.
...
Write the cultural review to
`production/localization/cultural-review-[date].md` as the declared output.
...
Read `Assets/Game/Localization/` (or the documented feature-local String Table
path) and `design/narrative/`. Identify:
- All dialogue lines (keys matching `dialogue.*`) with source text
- Lines already recorded (audio clip exists in `Assets/Game/Audio/VO/` or the
  documented Addressables audio location)
...
Write the approved VO script draft to
`production/localization/vo-scripts-[locale]-[date].md`; sending it to vendors
or recording remains an external high-risk action.

### VO Pipeline: Validate

Glob the project's declared VO directory under `Assets/Audio/VO/[locale]/` for
all `.wav`/`.ogg` files. Cross-reference against the VO manifest. Report:
...
Grep `Assets/Game/` and serialized reference reports for VO audio references.
Verify each referenced path exists under `Assets/Audio/VO/[locale]/`. Report broken references.
...
Read `.claude/docs/technical-preferences.md` and apply the configured Unity UI
system. Then check:

**Layout mirroring**
- Is RTL behavior implemented and tested for the project's UI Toolkit/uGUI and
  localization stack? Do not add an RTL package without dependency approval.
...
Write the RTL check report to
`production/localization/rtl-check-[date].md` as the declared output.
...
Write the localization QA report to
`production/localization/loc-qa-[locale]-[date].md` as the declared output.
