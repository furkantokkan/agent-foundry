...
`docs/reference/private/game-design/README.md` exists, read the index and use
targeted Grep for the GDD's domain to inform critique. Treat findings from the
reference library as advisory design heuristics, not project requirements.
...
- If the user accepts the status decision, update
  `design/gdd/systems-index.md` as part of that approval; the review remains
  read-only with respect to gameplay rules and implementation.
