...
Then use `AskUserQuestion` once to approve both the UX scope and its output
contract:
- "Approve this UX scope and output contract at
  `design/ux/[filename].md`? Accepted sections will be persisted there."
- Options: "Approve and start", "Add context first", "Pause"

This is the file-authorization boundary for the low-risk skeleton and accepted
sections. Do not add a second file-write prompt in fresh or retrofit mode.
...
Once the UX scope and output contract are approved, **immediately** create the
output file with empty section headers. The prior approval authorizes this
low-risk skeleton; do not ask again. This ensures incremental writes have a
target and work survives interruptions.
...
7. **Write**: after the user accepts the section's design decisions, persist it
   to `[filepath]` without a second file-write question.
...
3. **One scope approval, then persistence** — approve material UX decisions,
   then write the skeleton and accepted sections without per-section prompts
