---
name: prototype
description: "Unity-first concept prototype — validate the core idea before writing GDDs. Routes to Unity, HTML, or Paper based on the hypothesis and produces a PROCEED/PIVOT/KILL verdict."
argument-hint: "[concept-description] [--path unity|html|paper] [--review full|lean|solo] [--spike]"
user-invocable: true
...
Select the prototype path. If `--path [unity|html|paper]` was passed, use that.
Treat the historical `--path engine` spelling as an alias for `unity`; do not
activate a different engine or create a nested engine project.
Otherwise, use this quick-reference first, then read the full path details below:

| Genre | Recommended path | Key reason |
|-------|-----------------|------------|
| Platformer / action / fighter | **Unity** | Feel IS the hypothesis; browser latency produces false results |
| Racing / sports | **Unity** | Same — timing and physics feedback are the point |
| Top-down shooter / twin-stick | **Unity** | Aim feel is timing-sensitive |
...
| Roguelike (systems-heavy) | **Paper** → Unity | Validate that the ruleset is interesting before building |
...
| RPG / open world | **Paper** → Unity | Systems complexity: validate rules, then validate feel |
| Horror / atmospheric | **Unity** | Atmosphere requires real rendering |

**Rule of thumb:** "Does this feel right?" → Unity. "Are these rules interesting?" → Paper. "Is this logic correct?" → HTML or Paper.
...
**Distribution — the HTML path's biggest advantage:** Unlike Unity prototypes, this
...
### Path: Unity (isolated feature root in this project)
...
**Limitation — requires the pinned Unity Editor installed and running.** This path is a
multi-turn collaborative loop:
1. Agent writes the code
2. Unity automation validates compilation and the user or verifier playtests it
...
**Output:** A minimal runnable Unity prototype under
`Assets/Prototypes/[Name]Concept/**`, with focused tests under the canonical
`Assets/Tests/EditMode/**` and/or `Assets/Tests/PlayMode/**` roots. Evidence and
reports live under `production/prototypes/[name]-concept/**`.

Do not create a nested Unity project or switch to Godot, Unreal, Love2D, or another
runtime inside this repository. If Unity is too heavy for the hypothesis, choose
the HTML or Paper path; those outputs remain disposable documentation sandboxes,
not production runtime roots.
...
- **Ink** (Inkle) — plain-text scripting for branching narrative; its output can
  later be integrated through the project's approved Unity package stack
- **Yarn Spinner** — dialogue scripting for interactive conversations; package
  adoption remains a high-risk dependency decision
...
- **Options**:
  ...
  - `Unity — native prototype` — action, platformer, physics, or anything where feel IS the hypothesis. 50–60% one-shot; 2–4 iteration rounds are normal. Requires the pinned Unity Editor.
...
The plan must also declare exact allowed and forbidden paths, test/evidence paths,
the worktree/branch, and the serialized-asset writer. A new `.asmdef`, public API,
or cross-system contract is medium risk and needs plan approval. A scene, prefab,
ScriptableObject asset, Input Actions asset, package, Addressables topology, or
ProjectSettings change is high risk and needs exact approval. Once the plan is
approved, low-risk prototype code, tests, HTML/Paper files, and evidence inside
the declared paths do not require file-by-file prompts.
...
ends mid-build — especially important for multi-day Unity path work.

---

## Phase 5: Implement

After plan approval, create only the declared roots:

- Unity: `Assets/Prototypes/[Name]Concept/**` plus focused canonical test paths.
- HTML/Paper: `production/prototypes/[concept-name]-concept/sandbox/**`.
- Evidence/reporting: `production/prototypes/[concept-name]-concept/**`.

Every source or standalone prototype file must begin with the appropriate comment
syntax for:
...
> "The prototype files are written. Open the pinned Unity project now.
...
Write a single `prototype.html` to
`production/prototypes/[concept-name]-concept/sandbox/`. Include
...
Write `production/prototypes/[concept-name]-concept/sandbox/rules.md` (the game
rules) and `production/prototypes/[concept-name]-concept/sandbox/play-log.md` (a simulated session walking
...
> `production/prototypes/[name]-concept/sandbox/prototype.html` in your
...
**For Unity path:** The multi-turn iteration loop already captured errors and
...
> "Read through `production/prototypes/[name]-concept/sandbox/rules.md` and walk through the
...
Write the report to `production/prototypes/[concept-name]-concept/REPORT.md`
inside the approved prototype scope.

Then update `production/prototypes/index.md` (create if it does not
...
Link to `production/prototypes/[concept-name]-concept/REPORT.md`.
...
Write the approved decision to
`production/prototypes/[concept-name]-concept/PIVOT-NOTE.md` inside the
prototype documentation scope.

Write the file with: original hypothesis, what to keep, what to change, and the
revised hypothesis for the next prototype. When `/prototype` is next run, check
`production/prototypes/` for any `PIVOT-NOTE.md` files — if found, read them and use the
...
**Document the approved KILL verdict in `production/prototypes/GRAVEYARD.md`**
(create it if it does not exist) as part of the declared reporting output:
...
2. **Choose path** — same AskUserQuestion widget as Phase 3 (HTML / Unity / Paper).
...
6. **Write a spike note** (not a full report) to
   `production/prototypes/[concept-name]-spike-[date]/SPIKE-NOTE.md`:
   - Question tested
