...
Each agent receives allowed/forbidden paths and an output contract. Independent
planning artifacts may run in parallel; one compiler owns each final artifact.
Low-risk approved documents are autonomous, while economy/save/API contracts,
remote configuration, packages, and destructive live changes require explicit
medium/high-risk approval.
