...
Read relevant ScriptableObjects/data assets from `Assets/Game/Data/` (or the
project's documented feature-local data path) and formulas from
`design/balance/` for the identified domain.
...
- Guide the user to update the relevant asset under `Assets/Game/Data/` (or the
  documented feature-local data path) or formula in `design/balance/`
...
If [B]:
- Write the report to `design/balance/balance-check-[system]-[date].md` as the
  declared low-risk output of this invocation.
- If approved, write the report to that path (create the directory if needed). Use the current date for [date] in YYYY-MM-DD format.
