...
Always provide full context and an ownership contract in each agent's prompt:
base commit/worktree, allowed paths, forbidden paths, serialized assets, and
output contract. Parallel writing is allowed only for independent stories in
separate worktrees with disjoint paths. Work on one combat story hands off
sequentially, and each serialized Unity asset has one writer.
...
- Is the plain C#/MonoBehaviour/DOTS boundary justified and idiomatic for the pinned Unity stack?
...
### Phase 3: Implementation (ownership-gated)
Delegate sequentially unless each item is an independent story with its own
worktree and disjoint paths:
...
Low-risk files inside the approved story paths are autonomous. New public
contracts/assemblies require plan approval; serialized assets, project settings,
packages, save formats, and destructive operations require exact approval. The
orchestrator rejects overlapping writers and records the final diff/verification.
...
- Run `/game-code-review` on the implemented combat code before closing stories.
