...
`docs/reference/private/coding/README.md` exists, read the index and use targeted
Grep for patterns, dependency injection, async, Unity tooling, or library choices
that match the ADR domain. These references are supplementary; engine-reference
docs and accepted ADRs remain authoritative.
...
State Ownership:
  PlayerHealth → owned by Health system (ADR-0001)
  Interface: IHealthService.Current (read-only float)
  → If this ADR reads or writes player health, it must use this interface.

Interface Contracts:
  DamageDelivery → typed event contract (ADR-0003)
  Event: DamageApplied(DamageResult result)
  → If this ADR delivers or receives damage events, it must use this contract.

Forbidden Patterns:
  ✗ container/service-location inside domain code (ADR-0001)
  ✗ direct cross-system state write (ADR-0000)
...
| Field | Value |
|-------|-------|
| **Engine** | Unity [pinned version from ProjectVersion.txt] |
...
| **References Consulted** | [Pinned Unity/package docs, e.g. `docs/engine-reference/unity/VERSION.md`] |
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via Task to validate the drafted ADR:
   ...
   - If the Unity project/version cannot be validated, return `BLOCKED` rather than choosing another engine
   - Spawn `subagent_type: unity-specialist` with: the ADR's Engine Compatibility section, Decision section, Key Interfaces, and the Unity reference docs path. Ask them to:
...
5.7. **GDD Sync Check** — Before final ADR/propagation approval, scan all GDDs
referenced in the "GDD Requirements Addressed" section for naming inconsistencies
with the ADR's Key Interfaces and Decision sections (renamed typed events, API methods,
or data types). If any are found, surface them as a **prominent warning block**
immediately before the final decision — not as a footnote:
...
5. **Persist the ADR and gate cross-document propagation**:
...
If no GDD sync issues, write the approved ADR to
`docs/architecture/adr-[NNNN]-[slug].md` without a second file prompt.

If the cross-document update option is approved, write the ADR and the exact
reviewed GDD diff. If propagation is deferred, write only the ADR and record the
follow-up. Creating or widening unrelated files remains out of scope.
...
- Interface contracts it defines (typed event signatures, method APIs)
...
Registry candidates from this ADR:
  NEW state ownership:      PlayerStamina → Stamina system
  NEW interface contract:   StaminaDepleted typed event
  NEW performance budget:   stamina-system: 0.5ms/frame
  NEW forbidden pattern:    polling stamina each frame (use the approved event instead)
  EXISTING (referenced_by update only): PlayerHealth → already registered ✅
```
...
- Treat registry synchronization as part of the approved ADR plan; show the
  exact registry diff and update it only after that plan is approved.
