...
5. `.claude/docs/reference-libraries.md` if present. If
   `docs/reference/private/game-design/README.md` exists, read the index only
   and use targeted Grep for recurring review domains: loop, economy, pacing,
   pressure, fail-state, readability, dominant strategy, cognitive load, and
   level flow.
...
- Write the review to `design/gdd/gdd-cross-review-[date].md` as the declared
  read-only review artifact; do not mutate reviewed GDDs.
...
5. **Report autonomously; gate shared-state updates** — write the declared report
   without a duplicate prompt, but obtain plan approval before updating the
   systems index
