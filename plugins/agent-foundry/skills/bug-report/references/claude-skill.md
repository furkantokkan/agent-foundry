...
Before creating `production/qa/bugs/BUG-*.md`, search tracked task contracts by
exact ID when supplied and by objective, acceptance, preserved behavior, and
managed defect ledger. If the observation is a failure of an existing task's
acceptance, preserved behavior, or a regression caused by that task, do not
create a QA bug file. Forward all raw observations/evidence internally to:

```text
/task-cycle <TASK-ID> "<feedback>"
```

`task-cycle` is the sole same-task defect-ledger writer and will deduplicate,
repair, verify, reopen, or block the original task. If several task identities
are plausible, stop `BLOCKED` with one identity question. Continue this
standalone bug-report workflow only for an unrelated pre-existing defect or a
bug without a tracked task owner.
...
2. **Run the related test** — locate the mapped Unity test in
   `Assets/Tests/EditMode/`, `Assets/Tests/PlayMode/`, or a documented
   feature-local test assembly. Run it through the verified Unity automation
   route and report the NUnit XML result; do not infer PASS from console prose.
...
Update `production/qa/bugs/[BUG-ID].md` with the verified status as this mode's
declared QA-document operation; do not change production code.
...
After the closing criteria and decision are confirmed, mark
`production/qa/bugs/[BUG-ID].md` closed without a second file prompt.
...
Write `production/qa/bugs/BUG-[NNNN].md` as the declared low-risk output of the
report invocation.

Write the file, creating the directory if needed. Verdict: **COMPLETE** — bug
report filed. If required reproduction context is missing, return **BLOCKED**
for missing context rather than for file permission.
