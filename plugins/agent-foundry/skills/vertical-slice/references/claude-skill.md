...
The approved Phase 3 scope authorizes low-risk implementation inside the exact
declared `Assets/Prototypes/[ConceptName]VerticalSlice/**` allowlist and focused
tests under `Assets/Tests/EditMode/**` and/or `Assets/Tests/PlayMode/**`. Create
those paths inside the existing Unity project; never create a nested Unity
project. Stop for undeclared paths and apply the repository's medium/high-risk
approval gates, including the single-writer rule for serialized assets.

Every handwritten C# implementation file must begin with:
...
Write the report to
`production/prototypes/[concept-name]-vertical-slice/REPORT.md` as the declared
documentation output.

Write the approved report, then update `production/prototypes/index.md` (create
it if it does not exist) — append one row to the vertical slice table: concept name, date, verdict,
...
Link to `production/prototypes/[concept-name]-vertical-slice/REPORT.md`.
...
After the pivot decision is approved, write it to
`production/prototypes/[concept-name]-vertical-slice/PIVOT-NOTE.md` without a
second file prompt.

Write the file with: what worked, what failed, the specific systems or
...
`production/prototypes/` directory for a `PIVOT-NOTE.md` — use it to frame the new validation
...
After the KILL decision is approved, document it in
`production/prototypes/GRAVEYARD.md` (create it if it does not exist) without a
second file-permission prompt. Add one entry:
...
- Production code must NEVER import from `Assets/Prototypes/**` or from an
  HTML/Paper sandbox under `production/prototypes/**`
