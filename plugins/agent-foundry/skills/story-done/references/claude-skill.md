---
name: story-done
description: "Backward-compatible alias for task-done when given a task ID/contract path, plus legacy completion review for an explicit story file. Canonical tracked-task closure is /task-done."
argument-hint: "[task-id | contract-path | story-file-path] [--review full|lean|solo]"
user-invocable: true
...
Treat the complete block below as one raw closure input. Preserve the exact ID,
path, and review override.

<story-done-input>
$ARGUMENTS
</story-done-input>

For a tracked task, forward raw input to `/task-done` and stop. For an explicit
legacy story path, retain the old completion review.

**Output:** Task status closure for an ID, or legacy story completion for an
explicit story path.

---

## Phase 1: Resolve the task or story
...
1. A complete task ID resolves only as
   `production/tasks/<TASK-ID>/contract.md`. Never fall back to partial matches,
   timestamps, or a similarly named epic/story.
2. An explicit in-repository `production/tasks/<ID>/contract.md` path selects
   the task contract closure lane below.
3. An explicit story-file path selects the legacy story lane beginning at
   Phase 2.
4. Missing/ambiguous/outside-repository input stops before evidence collection
   or writes and names the exact problem.

If a task contract was resolved, run **Task Contract Closure Lane** and stop;
do not run the legacy story phases.

For the legacy story lane, resolve the review mode once:
...
**If a story file path is provided** (for example,
`/story-done production/epics/core/story-damage-calculator.md`), read it
directly.
...
For a complete task ID or exact task-contract path, invoke `/task-done` with the
raw input and stop. Do not add another close confirmation, read/write
`status.md`, or enter the legacy phases below. `task-done` owns the canonical
lifecycle block, evidence freshness, idempotency, and safe reopen contract.
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **EditMode test**: check `Assets/Tests/EditMode/` and feature-local EditMode
     assemblies for a test file or method name that
     matches the criterion's subject (use `Glob` and `Grep`)
   - **PlayMode/integration test**: check `Assets/Tests/PlayMode/` and
     feature-local integration assemblies similarly
...
|-----------|------|--------|
| AC-1: [criterion text] | Assets/Tests/EditMode/FooTests.cs::Scenario_Result | COVERED |
...
|---|---|---|
| **Logic** | Unity EditMode test in `Assets/Tests/EditMode/` or feature-local test assembly — must exist and pass | BLOCKING |
| **Integration** | Unity PlayMode/EditMode integration test OR playtest doc | BLOCKING |
...
found, also search Unity EditMode test assemblies broadly (the file may have been placed at a
...
required path. Use `Glob` to check that exact path first, then search Unity
PlayMode/EditMode integration assemblies broadly, then check `production/session-logs/` for a
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via Agent using gate **QL-TEST-COVERAGE** (`.claude/docs/director-gates.md`).
...
- `lean` → use `AskUserQuestion` before proceeding:
  - Prompt: "Code review is skipped in lean mode. Did you run `/game-code-review` on the implemented files?"
  - Options:
    - `Yes — /game-code-review passed or was approved with suggestions`
    - `No — skipping code review for this story`
    - `No — I'll run /game-code-review before the sprint close-out`
  - Record the answer in the completion notes (Phase 7). All three options proceed to Phase 6.
- `full` → spawn as normal.

Spawn `lead-programmer` via Agent using gate **LP-CODE-REVIEW** (`.claude/docs/director-gates.md`).
...
**Base commit**: [sha]
**Allowed paths**: [list]
**Forbidden-path violations**: [None or list]
**Serialized assets touched**: [list with single writer, or None]
...
If the verdict is **BLOCKED**: do not proceed to Phase 7. List the exact failing
test/criterion and evidence. When a reproducible bug exists inside the story's
approved boundary, hand it sequentially to `unity-bugfixer`, then return the
result to `unity-test-verifier`. Do not let either role overlap writes or broaden
paths. Architectural/ownership expansion requires a new approved plan/story.
...
```
Suggested commit:
git add [Assets/ and related approved files changed during implementation] [story-file-path]
...
- **Never let the verifier fix production code** — production failures follow
  the explicit bugfixer handoff and focused reverification loop.
- **Deviations are facts, not judgments** — present them neutrally; the user
  decides if they are acceptable.
- **BLOCKED is a quality gate** — closing anyway requires an explicit written
  waiver identifying the failing criterion, owner, risk, and follow-up story.
