...
### Unity Ownership and Risk

- [ ] **Allowed paths declared**: The story lists the exact code, test, data,
  and documentation path globs the implementer may write.
- [ ] **Forbidden paths declared**: The story identifies protected adjacent
  systems and out-of-scope paths. `None` is valid only with a reason.
- [ ] **Worktree/base declared**: Code-writing stories name the base branch or
  commit and require an isolated story worktree.
- [ ] **Serialized assets owned**: Every referenced scene, prefab,
  ScriptableObject, `.inputactions` asset, Addressables group, or project setting
  has one named writer, or the story explicitly states that none are touched.
- [ ] **Risk tier declared**: The story classifies its expected work as low,
  medium, or high risk and lists the approval gate for medium/high-risk actions.
- [ ] **Handoff destination declared**: The story names the next role and the
  evidence it must receive, normally implementer -> verifier -> bugfixer.
...
- [ ] **Referenced assets exist**: Scan the story text for asset path patterns
  (paths containing `Assets/`, or Unity-relevant extensions such as `.png`,
  `.jpg`, `.svg`, `.wav`, `.ogg`, `.mp3`, `.fbx`, `.prefab`, `.unity`, `.asset`,
  `.controller`, `.inputactions`, `.uxml`, `.uss`, or `.shadergraph`).
...
Spawn `qa-lead` via Agent using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
