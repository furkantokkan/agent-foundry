---
name: create-stories
description: "Legacy-only epic-to-story workflow for repositories that explicitly retain separate story files. Do not use in the canonical task-first workflow, where /create-task already creates the implementable and closable contract."
...
This is a compatibility command. First read repository workflow authority. If
it selects the canonical task-first model, stop without creating files and
route each independently closable epic outcome directly to `/create-task`.
Continue below only when the repository explicitly retains
`production/epics/**/story-*.md` as executable authority.
...
After decomposing all stories (Step 4 complete) but before presenting them for
scope/content approval, spawn `qa-lead` via Task using gate
**QL-STORY-READY** (`.claude/docs/director-gates.md`).
...
Story 001: [title] — Logic — ADR-NNNN
  ...
  Test required: Assets/Tests/EditMode/[System]/[StoryName]Tests.cs

Story 002: [title] — Integration — ADR-MMMM
  Covers: TR-[system]-002, TR-[system]-003
  Test required: Assets/Tests/PlayMode/[System]/[StoryName]Tests.cs
...
Use `AskUserQuestion`:
- Once the story set is approved, write it to
  `production/epics/[epic-slug]/` without per-file prompts.
...
**Engine**: Unity 6.3 LTS | **Engine Risk**: [LOW / MEDIUM / HIGH]
**Engine Notes**: [from ADR Engine Compatibility section — post-cutoff APIs, verification required]

**Change Risk**: [LOW / MEDIUM / HIGH]
**Approval Gate**: [None for low risk | plan approval | explicit high-risk approval]
**Base Branch/Commit**: [branch and SHA]
**Story Worktree**: [required name for code/asset-writing stories]
...
**Allowed Paths**:
- `Assets/Game/[feature]/**`
- `Assets/Tests/EditMode/[feature]/**` or the required PlayMode path
- [approved documentation/evidence paths]

**Forbidden Paths**:
- [adjacent systems and protected paths, or "None — reason"]

**Serialized Asset Ownership**:
- [scene/prefab/ScriptableObject/.inputactions/Addressables/project setting]
- [single writer role, or "None — this story changes code/tests only"]

**Handoff**:
- Implementer: `unity-feature-implementer` or assigned Unity programmer
- Verifier: `unity-test-verifier`
- Bugfix owner: `unity-bugfixer`
...
**Story Type**: [type]
**Required evidence**:
- Logic: `Assets/Tests/EditMode/[system]/[StoryName]Tests.cs` — must exist and pass
- Integration: `Assets/Tests/PlayMode/[system]/[StoryName]Tests.cs`, a
  feature-local Unity integration assembly, OR an approved playtest doc
...
4. **Approve the story set once** — after scope/content approval, persist the
   declared story files without a per-file loop
