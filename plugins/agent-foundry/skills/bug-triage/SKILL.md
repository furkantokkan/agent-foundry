...
Codex port of Claude Code Game Studios /bug-triage.
...
Before assigning a duplicate, reconcile each report with tracked task
acceptance, preserved behavior, and defect ledgers. Route owned same-task
failures to `$task-bug [<ID>] "<feedback>"` as `TRACKED_TASK_DEFECT`;
`task-bug` resolves/dispatches and `task-cycle` alone creates/deduplicates
stable linked `D-xxx` rows and records. Keep standalone
triage only for unowned or genuinely separate bugs.
