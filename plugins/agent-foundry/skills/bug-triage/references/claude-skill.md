...
### Step 2d — Reconcile tracked-task ownership

For each open bug, search `production/tasks/*/contract.md` by an explicit task
ID and by objective, acceptance, preserved behavior, and managed defect ledger.
If it is a failure owned by an existing task, do not assign a duplicate task or
rewrite that contract. Classify it in the report as `TRACKED_TASK_DEFECT` with
next action `/task-cycle <TASK-ID> "<raw observation>"`. `task-cycle` alone
deduplicates and writes stable `D-xxx` lifecycle rows. Keep standalone backlog
triage only for bugs without a tracked task owner or genuinely separate scope.
...
- If it is a `TRACKED_TASK_DEFECT`, retain the original task as owner and route
  it to `task-cycle`; do not create or assign a replacement story/task.
...
Write the triage report to `production/qa/bug-triage-[date].md` as the declared
low-risk output; no separate per-file confirmation is required.
...
If ownership or the declared report path is unavailable: Verdict: **BLOCKED** —
