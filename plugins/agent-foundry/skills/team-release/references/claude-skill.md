...
Each agent receives allowed/forbidden paths and an output contract. Verification
may run in parallel only when read-only or isolated. Approved release documents
are low risk; branch protections, deployment configuration, packages, player
data/save migration, store publishing, and destructive operations require exact
high-risk approval.
