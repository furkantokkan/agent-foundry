---
name: create-epics
description: "Optionally group a large approved GDD/architecture outcome into an epic containing several independently closable task contracts. Use for roadmap-scale planning, not routine bugs or bounded features; task-first projects create tasks directly after the epic."
...
An epic is an optional named grouping for several independently closable tasks.
It defines a large outcome and its architectural boundaries, but is never
implemented directly. The task contracts prescribe executable scope.
...
**Next step after each epic:** create one `/create-task` contract per
independently verifiable deliverable. Do not add story files unless repository
instructions explicitly select the legacy story pipeline.
...
- `docs/engine-reference/unity/VERSION.md` — engine name, version, risk levels
...
After approval, write `production/epics/[epic-slug]/EPIC.md` as this skill's
declared low-risk output without a duplicate file prompt.
...
- All Logic and Integration stories have passing Unity tests in
  `Assets/Tests/EditMode/`, `Assets/Tests/PlayMode/`, or documented
  feature-local test assemblies, with NUnit XML evidence
...
3. **Approve epic scope once** — obtain approval for the epic set and material
   decisions, then persist all declared epic files without per-file prompts
