...
4. **Persist approved design decisions:**
   - Show the draft section or decision summary for creative approval
   - After the user accepts the decision, write it to the declared documentation
     path without a second per-file confirmation
   - Ask again only when the target path/scope changes or a medium/high-risk
     boundary in the canonical collaboration policy is crossed
...
All tuning knobs must live in project-owned ScriptableObject/data assets under
`Assets/Game/Data/` or a documented feature-local path, never hardcoded.
Document the intended range and the reasoning for the current value.
