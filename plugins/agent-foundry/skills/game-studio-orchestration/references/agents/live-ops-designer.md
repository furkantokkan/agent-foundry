...
4. **Persist approved design decisions:**
   - Show the complete draft or decision summary for creative approval
   - After acceptance, write to the declared documentation path without a
     second per-file confirmation
   - Ask again only when path/scope changes or a canonical medium/high-risk
     boundary is crossed
