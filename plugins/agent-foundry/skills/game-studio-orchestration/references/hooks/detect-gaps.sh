...
# Check if Unity game code exists
if [ -d "Assets/Game" ]; then
  SRC_CHECK=$(find Assets/Game -type f -name "*.cs" 2>/dev/null | head -1)
  if [ -n "$SRC_CHECK" ]; then
    FRESH_PROJECT=false
  fi
fi
...
if [ -d "Assets/Game" ]; then
  # Count project-owned Unity C# files; exclude packages and vendor assets.
  SRC_FILES=$(find Assets/Game -type f -name "*.cs" 2>/dev/null | wc -l)
else
  SRC_FILES=0
fi
...
if [ "$SRC_FILES" -gt 50 ] && [ "$DESIGN_FILES" -lt 5 ]; then
  ...
  echo "    Suggested action: /reverse-document design Assets/Game/[System]"
  echo "    Or run: /project-stage-detect to get full analysis"
fi

# --- Check 2: Prototypes without documentation ---
if [ -d "Assets/Prototypes" ]; then
  PROTOTYPE_DIRS=$(find Assets/Prototypes -mindepth 1 -maxdepth 1 -type d 2>/dev/null)
  UNDOCUMENTED_PROTOS=()
  ...
  if [ -n "$PROTOTYPE_DIRS" ]; then
    ...
    if [ ${#UNDOCUMENTED_PROTOS[@]} -gt 0 ]; then
      ...
      for proto in "${UNDOCUMENTED_PROTOS[@]}"; do
        echo "    - Assets/Prototypes/$proto/ (no README or CONCEPT doc)"
      done
      echo "    Suggested action: /reverse-document concept Assets/Prototypes/[Name]"
    fi
  fi
fi

# --- Check 3: Core systems without architecture docs ---
if [ -d "Assets/Game/Core" ] || [ -d "Assets/Game/Engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    ...
    if [ "$ADR_COUNT" -lt 3 ]; then
      ...
      echo "    Suggested action: /reverse-document architecture Assets/Game/Core/[System]"
    fi
  fi
fi

# --- Check 4: Gameplay systems without design docs ---
if [ -d "Assets/Game/Gameplay" ]; then
  # Find major gameplay subdirectories (those with 5+ files)
  GAMEPLAY_SYSTEMS=$(find Assets/Game/Gameplay -mindepth 1 -maxdepth 1 -type d 2>/dev/null)
    ...
    while IFS= read -r system_dir; do
      ...
      if [ "$file_count" -ge 5 ]; then
        ...
        if [ ! -f "$design_doc_1" ] && [ ! -f "$design_doc_2" ]; then
          echo "⚠️  GAP: Gameplay system 'Assets/Game/Gameplay/$system_name/' ($file_count files) has no design doc"
          echo "    Expected: design/gdd/${system_name}-system.md or design/gdd/${system_name}.md"
          echo "    Suggested action: /reverse-document design Assets/Game/Gameplay/$system_name"
        fi
      fi
