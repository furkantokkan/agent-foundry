...
# Open bug count. Search the canonical bug directory once; searching its parent
# as well would count the same report twice.
BUG_COUNT=0
...
if [ -d "Assets/Game" ]; then
    TODO_COUNT=$(grep -r --include="*.cs" "TODO" Assets/Game/ 2>/dev/null | wc -l)
    FIXME_COUNT=$(grep -r --include="*.cs" "FIXME" Assets/Game/ 2>/dev/null | wc -l)
    if [ "$TODO_COUNT" -gt 0 ] || [ "$FIXME_COUNT" -gt 0 ]; then
        echo ""
        echo "Code health: ${TODO_COUNT} TODOs, ${FIXME_COUNT} FIXMEs in Assets/Game/"
    fi
fi
