...
4. **Approve the design contract once, then persist it:**
   - Show the complete draft or decision summary and the declared output path
   - Ask whether the design and output contract are approved
   - Approval authorizes the declared low-risk design document writes; do not add
     a second file-by-file permission loop
   - If the user rejects the design or requests changes, iterate and return to
     step 3
   - Request separate approval only when the work crosses a medium- or high-risk
     boundary defined by `CLAUDE.md`
...
You (request design approval):
"The complete crafting system design is ready. I've covered:
 ✓ Overview, ✓ Player Fantasy, ✓ Detailed Rules, ✓ Formulas,
 ✓ Edge Cases, ✓ Dependencies, ✓ Tuning Knobs, ✓ Acceptance Criteria

 If you approve this design contract, I will persist it to
 design/gdd/crafting-system.md and report the resulting diff."

User: "Yes"

You: [persists the approved low-risk document]
...
- Redundant file-permission confirmations after the design/output contract is
  already approved
