...
|----------|---------------|-----------------|--------|-------|
| Xbox (GDK) | Current Xbox Accessibility Guidelines and platform/NDA requirements | [Remapping, contrast, narration/menu support as verified for target program] | Not Started | Verify the current official XAG and certification requirements for the exact program/release; do not infer certification status from this template. |
| PlayStation 5 | Current Sony platform accessibility and certification documentation | [Screen-reader/menu, audio, contrast features supported by the approved Unity/platform bridge] | Not Started | Exact APIs and requirements must come from current authorized platform documentation; this public template does not assert an API contract. |
...
| PC (Screen Reader) | JAWS / NVDA / Windows Narrator | [Menu navigation announcements] | Not Started | Requires an approved Unity/platform accessibility bridge that exposes names, roles, states, and navigation. Verify the pinned Unity/package and OS support; UI Toolkit/UGUI alone is not proof. |
...
|---------|--------------|-----------------|--------------|------------|
| [Screen reader support for in-game world (NPCs, objects, environmental text)] | Exemplary | The approved Unity/platform accessibility bridge currently covers only the verified menu scope; spatial world description requires separate design and implementation | Affects blind and low-vision players who can navigate menus but cannot independently explore the game world | Duplicate critical information in accessible menus and evaluate a tested spatial description solution |
| [Full subtitle customization (font/color/background)] | Comprehensive | Scope reduction — targeting Standard tier. The approved Unity localization/font/UI pipeline needs additional authored variants and validation | Affects deaf and hard-of-hearing players with specific legibility needs; particularly affects players with dyslexia who use custom fonts | Provide verified high-readability presets as partial mitigation; track the gap explicitly |
...
|----------|-------|----------|-----------|
| [Which pinned Unity/package/platform bridge exposes dynamic accessible name, role, state, and HUD announcements on each target platform?] | [ux-designer + unity-ui-specialist] | [Before Technical Setup gate] | [Unresolved — verify official pinned docs and a fixture build] |
