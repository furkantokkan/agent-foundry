...
- Read `.claude/docs/reference-libraries.md`, then
  `docs/reference/private/game-design/README.md` if present. Use the index to
  choose targeted Grep terms for relevant loop, pacing, economy, risk/reward,
  fail-state, or level-design references. These references are advisory; the
  current concept and approved GDDs remain authoritative.
...
Write the approved systems index to `design/gdd/systems-index.md` as the
declared output of this invocation.

Wait for priority/scope approval. Then write the declared index without a
second file-permission question.
...
2. **AskUserQuestion** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 5: persist the approved systems index
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `/design-system`
3. **Scope approval once** before writing the declared index; no per-file loop
